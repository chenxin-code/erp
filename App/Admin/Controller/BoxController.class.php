...
class BoxController extends BaseController{
    ...
    public function add(){
        $BOrdCardSelect = M()->table('BOrdCard')->where(['CusId' => 'TAOBAO'.$this->config['FactoryId'],'Checked' => 1])->field('MatNo')->select();
        foreach($BOrdCardSelect as $k => $v){
            unset($BOrdCardSelect[$k]['ROW_NUMBER']);
        }
        $this->assign([
            'LayoutTitle' => '添加淘宝箱',
            'BOrdCardSelect' => jejuu($BOrdCardSelect),
        ]);
        $this->display();
    }
    ...
    public function add_api(){
        ...
        if($get['DefaultPic'] === '1'){
            $ProductData['Pic'] = $this->config['BoxDefaultPic'];
        }
        if($get['IsRangePrice']){
    ...
    public function edit(){
        ...
        $thisProductInfo = M()->table('WebProduct')
            ...
            ->field('MatNo,Title,IsRangePrice,Price,MarketPrice,BuildMin,BuildMax,CusMax,Total,BeginTime,EndTime,IsFlag,IsDel')
            ->find();
        if(!$thisProductInfo){
            $this->error('参数错误');die;
        }
    ...
    public function addDefaultPic_api(){
        $BoxDefaultPic = $this->config['BoxDefaultPic'];
        ...
        if($BoxDefaultPic){
            $BoxDefaultPic = $BoxDefaultPic.','.$r1['images'];
        }else{
            $BoxDefaultPic = $r1['images'];
        }
        $r2 = D('Func')->aosConfig('BoxDefaultPic',$BoxDefaultPic);
        if($r2){
            echo jejuu([
                'ret' => C('succ_ret'),
                'msg' => '添加成功',
    ...
    public function delDefaultPic_api(){
        ...
        $BoxDefaultPic = str_replace(','.$get['Pic'],'',$BoxDefaultPic);
        $BoxDefaultPic = str_replace($get['Pic'].',','',$BoxDefaultPic);
        $BoxDefaultPic = str_replace($get['Pic'],'',$BoxDefaultPic);
        $r = D('Func')->aosConfig('BoxDefaultPic',$BoxDefaultPic);
        if($r){
            //默认图片没有被使用才能删除
    ...
    public function showDefaultPic_api(){
        $BoxDefaultPic = $this->config['BoxDefaultPic'];
        $BoxDefaultPic = $BoxDefaultPic?explode(',',$BoxDefaultPic):[];
        echo jejuu($BoxDefaultPic);
    }
    ...
    public function delPic_api(){
        ...
        if($r !== FALSE){
            ...
            $BoxDefaultPic = $BoxDefaultPic?explode(',',$BoxDefaultPic):[];
            //不是默认图片且没被使用才能删除
            if(!in_array($get['Pic'],$BoxDefaultPic,true) && !(M()->table('WebGroupOrder')->where(['FirstPic' => ['like','%'.$get['Pic'].'%']])->select())){
                unlink('./res/'.$get['Pic']);
            }
            echo jejuu([
                'ret' => C('succ_ret'),
    ...
    public function changeFlag_api(){
        ...
        if($wp['IsFlag'] === '0'){
            $IsFlag = '1';
            $msg = '设置“'.$this->config['BoxFlag'].'”';
        }else{
            $IsFlag = '0';
            $msg = '取消“'.$this->config['BoxFlag'].'”';
        }
